...
rent_values = np.array([*range(2, 52, 2), 25, 25, 25, 25, 28, 28]) * 5
...
# Nodes
player = Node(FiringMode.PASSIVE, initial_resources=[(money, 1500),(estate,0)])
...
# Edges
player_gets_go = ResourceConnection(bank, player, money, 200)
...
estate_rent_modifier = Modifier(player, player_gets_rent, estate, money, 2000)
get_rent = Trigger(get_rent_odds, player_gets_rent, luck, Predicate("<=", 40), TriggerMode.AUTOMATIC)
...
m = Machinations.load((
    ...
    [player_gets_go, pass_go, player_pays_rent, how_much_rent, pay_rent, player_buys_estate, buy_trigger, estate_modifier, player_gets_rent, estate_rent_modifier, get_rent, likelier_to_get_rent, lose],
    [money, estate, luck],
))
